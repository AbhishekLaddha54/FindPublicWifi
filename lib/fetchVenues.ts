...
export async function fetchVenuesClientSide(lat: number, lon: number, radius: number): Promise<Venue[]> {
  try {
    // Fetch venues from OpenStreetMap Overpass API directly from client
    const osmVenues = await fetchOSMVenues(lat, lon, radius)

    // Add mock Wi-Fi data
    const wifiData = getMockWiFiData()
